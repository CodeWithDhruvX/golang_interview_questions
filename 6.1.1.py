...
def load_transcript_with_timestamps(transcript_file, input_video):
    """Load plain-text transcript and generate estimated timestamps."""
    lines = []
    try:
    ...
    if not text_lines:
        logging.warning("Transcript is empty.")
        return []

    try:
        video_duration = get_video_duration(input_video)
        duration_per_line = video_duration / len(text_lines)
    except Exception as e:
        logging.warning(f"Failed to get duration; using default timing. Error: {e}")
        duration_per_line = 3.0
        video_duration = duration_per_line * len(text_lines)

    for i, text in enumerate(text_lines):
        start_time = i * duration_per_line
        end_time = min((i + 1) * duration_per_line, video_duration)
        lines.append({
...
in this code , please ask the input after the video selection, choose transcript folder and based on the filename matching with video trasncription text add as subtitle and give me the final version of the code