...
class YouTubeCommentManager:
    def __init__(self, root):
        ...
        self.duration_filter = tk.StringVar()
        ...
        self.duration_filter.trace('w', self.apply_filters)
    ...
    def create_widgets(self):
        ...
        # Duration filter
        ttk.Label(filter_frame, text="Max Duration (sec):").grid(row=1, column=3, padx=5, pady=2, sticky='w')
        duration_entry = ttk.Entry(filter_frame, textvariable=self.duration_filter, width=15)
        duration_entry.grid(row=1, column=4, padx=5, pady=2, sticky='ew')
        
        # Clear filters button
        ttk.Button(filter_frame, text="🗑️ Clear Filters", 
                  command=self.clear_filters).grid(row=1, column=5, padx=5, pady=2)
