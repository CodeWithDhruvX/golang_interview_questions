...
def run_subprocess_safe(cmd, timeout=300, **kwargs):
    """FIXED: Run subprocess with proper encoding handling and resource management"""
    ...
    try:
        ...
        if check_flag and proc.returncode != 0:
            # FIXED: Better error message formatting with encoding safety
            cmd_str = ' '.join(str(x) for x in cmd[:5])
            error_msg = f"Command failed with return code {proc.returncode}: {cmd_str}"
            if stderr:
                # Safely truncate stderr to avoid encoding issues
...
def get_video_duration(file_path):
    """Get video duration with better error handling and encoding safety"""
    try:
        result = run_subprocess_safe([
            'ffprobe', '-v', 'error', '-show_entries', 'format=duration', 
            '-of', 'json', str(file_path)  # Ensure string conversion
        ], timeout=30)
...
def transcribe_audio_fixed(audio_path, progress_callback=None, max_duration=1800):
    """
    FIXED: Transcription with proper resource management and encoding safety
    """
    ...
    model = None
    try:
        # Get audio duration with validation and encoding safety
        try:
        ...
        if duration <= 180:  # 3 minutes or less
            ...
            try:
                segments, _ = model.transcribe(
                    str(audio_path),  # Ensure string conversion
                    beam_size=5, 
            ...
            for i in range(max_chunks):
                ...
                # Create unique temporary file with safe naming
                chunk_path = f"temp_chunk_{i}_{int(time.time())}_{os.getpid()}.wav"
                TEMP_FILES.append(chunk_path)
                
                try:
                    # Extract chunk with strict timeout and encoding safety
                    run_subprocess_safe([
...
def group_words_into_subtitles_fixed(words, max_words_per_group=4, min_duration=1.5, max_duration=4.0, min_gap=0.3):
    """
...
def safe_text_escape(text):
    """FIXED: Safely escape text for ASS format with encoding handling"""
...
def generate_highlighted_subtitle_ass_fixed(subtitle_groups, ass_path):
    """
    FIXED: Generate highlighted subtitle ASS with better encoding and formatting
    """
    try:
...
""")
            ...
            for i, group in enumerate(subtitle_groups):
                # Clean and format text with safe escaping
                ...
                formatted_text = f"{{\\fad(200,200)\\bord2\\shad1\\c&HFFFFFF&\\3c&H000000&\\4c&H000000&}} {text}"
...
def get_title_for_video(input_video):
    """Get title for video from mapping with encoding safety"""
...
def generate_hello_world_ass(ass_path, video_duration, title_text):
    """FIXED: Generate title overlay ASS with encoding safety"""
    try:
        with open(ass_path, "w", encoding="utf-8", errors='replace') as f:
            f.write("""[Script Info]
Title: Title Overlay
...
def add_background_music_simple(video_path, music_path, output_path, music_volume=0.15):
    """
    FIXED: Simplified background music addition with encoding safety
    """
    try:
        run_subprocess_safe([
...
def process_video_with_settings(input_video, output_video, background_music, subtitle_individual, subtitle_highlighted, add_title, use_auto_editor, use_transcription, progress_callback=None, min_subtitle_duration=1.5, max_words_per_subtitle=4):
    """
    FIXED: Main video processing function with encoding safety and custom settings
    """
    ...
    try:
        ...
        if use_auto_editor:
            ...
            try:
                # FIXED: Better auto-editor command with encoding safety
                run_subprocess_safe([
        ...
        if use_transcription and (subtitle_individual or subtitle_highlighted or background_music):
            ...
            # Extract audio with timeout and encoding safety
            run_subprocess_safe([
        ...
        if subtitle_highlighted or add_title:
            ...
            # FIXED: Use ASS subtitles with proper path handling
            filter_parts = []
            if subtitle_highlighted and use_transcription and subtitle_groups:
                # FIXED: Escape path for filter
...
class VideoProcessorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Video Processor - Fixed Encoding & Subtitle Timing")
    ...
    def create_widgets(self):
        ...
        self.individual_cb = ttk.Checkbutton(
            subtitle_frame, 
            text="📝 Individual Word Subtitles (Disabled - Encoding Issues)", 
        ...
        self.highlighted_cb = ttk.Checkbutton(
            subtitle_frame, 
            text="🔤 Highlighted Group Subtitles (FIXED - Better Timing & Encoding)", 
        ...
        self.max_words_var = tk.IntVar(value=4)
    ...
    def process_videos(self):
        """FIXED: Process all selected videos with improved encoding and subtitle settings"""
        successful = 0
        failed = 0
        ...
        try:
            for i, input_video in enumerate(self.input_videos, 1):
                ...
                try:
                    ...
                    base_name = Path(input_video).stem
                    safe_base_name = re.sub(r'[^\w\-_\.]', '_', base_name)
                    output_path = os.path.join(self.output_dir, f"{safe_base_name}_processed.mp4")
                    ...
                    while os.path.exists(output_path):
                        output_path = os.path.join(
                            self.output_dir, 
                            f"{safe_base_name}_processed_{counter}.mp4"
                        )
                        counter += 1
                    ...
                    process_video_with_settings(
                        ...
                        subtitle_individual=False,  # Disabled to prevent encoding issues
...
def main():
    ...
    try:
        ...
        # Start the GUI
        logging.info("🚀 Video Processor started with FIXED encoding & subtitle timing")
        root.mainloop()
