...
class OptimizedVideoProcessor:
    ...
    def generate_ass_subtitles_enhanced(self, content, ass_path, settings):
        ...
        try:
            ...
            style_line = f"Style: Default,{font_name},{subtitle_size},{self.hex_to_ass_color(subtitle_color)},{secondary},{outline_color},{back},{bold},{italic},{underline},{strikeout},{scale_x},{scale_y},{spacing},{angle},{border_style},{outline},{shadow},{alignment},{margin_l},{margin_r},{margin_v},{encoding}"

            style_border = ""
            if settings['enable_borders']:
                size_border = str(subtitle_size + 2)
                outline_border = str(int(outline) + 1) if outline != "0" else "1"
                style_border = f"\nStyle: SpeechBorder,{font_name},{size_border},{self.hex_to_ass_color(subtitle_color)},{secondary},{outline_color},{back},{bold},{italic},{underline},{strikeout},{scale_x},{scale_y},{spacing},{angle},{border_style},{outline_border},{shadow},{alignment},{margin_l},{margin_r},{margin_v},{encoding}"
                    ...
                    for w in content:
                        ...
                        try:
                            ...
                            if word and end > start:
                                primary_color = self.hex_to_ass_color(random.choice(self.random_colors_list)) if random_colors else self.hex_to_ass_color(subtitle_color)
                                style_line = f"Style: Default,{font_name},{subtitle_size},{primary_color},{secondary},{outline_color},{back},{bold},{italic},{underline},{strikeout},{scale_x},{scale_y},{spacing},{angle},{border_style},{outline},{shadow},{alignment},{margin_l},{margin_r},{margin_v},{encoding}"
                                if settings['enable_borders']:
                                    style_border = f"Style: SpeechBorder,{font_name},{size_border},{primary_color},{secondary},{outline_color},{back},{bold},{italic},{underline},{strikeout},{scale_x},{scale_y},{spacing},{angle},{border_style},{outline_border},{shadow},{alignment},{margin_l},{margin_r},{margin_v},{encoding}"
                                f.write(f"[V4+ Styles]\n{format_str}\n{style_line}{style_border if settings['enable_borders'] else ''}\n\n[Events]\nFormat: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text\n")
                                animation_tag = self.get_animation_tag(animation_type, start, end) if enable_animation else ""
                                text = f"{{\\{animation_tag}}}{word}" if animation_tag else word
                    ...
                    for sub in grouped_subs:
                        ...
                        try:
                            ...
                            if text and end > start:
                                ...
                                f.write(f"[V4+ Styles]\n{format_str}\n{style_line}{style_border if settings['enable_borders'] else ''}\n\n[Events]\nFormat: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text\n")
                                animation_tag = self.get_animation_tag(animation_type, start, end) if enable_animation else ""
                                sub_text = f"{{\\{animation_tag}}}{text}" if animation_tag else text
            ...
            for idx, video_path in enumerate(input_videos, 1):
                ...
                try:
                    ...
                    if self.extract_audio(video_path, temp_audio):
                        ...
                        if not self.encode_video_with_subtitles(current_video, temp_subtitle, final_output, quality, use_gpu):
                    ...
                    for temp_file in [temp_audio, temp_video, temp_subtitle, merged_path if extra_video else None, music_output if background_music else None, auto_edited_path if enable_auto_edit else None]:
...
if __name__ == "__main__":
    try:
        ...
        please give me the more animation style on the subtitles for the youtube videos and if you have idea that which subtitle animation that popular in the youtube 